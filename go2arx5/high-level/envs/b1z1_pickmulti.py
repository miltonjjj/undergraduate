...
class B1Z1PickMulti(B1Z1Base):
    ...
    def _setup_obs_and_action_info(self):
        super()._setup_obs_and_action_info(removed_dim=9, num_action=9, num_obs=38+self.num_features-1)#arx5go2=40
    ...
    def _create_extra(self, env_i):
        ...
        cube_start_pose.p.z = self.table_heights[i] + obj_height + 0.01
    ...
    def _reset_objs(self, env_ids):
        ...
        self._cube_root_states[env_ids, 2] = self.table_heights[env_ids] + self.init_height[env_ids]+0.02
        #rand_yaw_box = torch_rand_float(-3.15, 3.15, (len(env_ids), 1), device=self.device).squeeze(1)
        rand_yaw_box=torch.tensor(0.0, device=self.device)
    ...
    '''
    def _reset_objs(self, env_ids):
        """重写父类方法,强制只使用blue_tea_box"""
        # 获取blue_tea_box在配置中的索引
        tea_box_idx = self.obj_list.index("blue_tea_box")
        
        for i in env_ids:
            # 获取桌面高度
            table_height = self.table_heights[i]
            
            # 强制使用blue_tea_box
            obj_asset = self.ycb_asset_list[tea_box_idx]
            obj_height = self.obj_height[tea_box_idx]
            
            # 设置物体位置
            self._cube_root_states[i, 0] = self._table_root_states[i, 0] + np.random.uniform(-0.1, 0.1)
            self._cube_root_states[i, 1] = self._table_root_states[i, 1] + np.random.uniform(-0.1, 0.1)
            self._cube_root_states[i, 2] = table_height + obj_height + 0.01
            
            # 设置物体朝向
            #rand_yaw_box = np.random.uniform(-np.pi, np.pi)
            #rand_r_box = np.random.uniform(-0.05, 0.05)
            #rand_p_box = np.random.uniform(-0.05, 0.05)
            rand_yaw_box = torch.tensor(0.0, device=self.device)
            rand_r_box = torch.tensor(0.0, device=self.device)
            rand_p_box = torch.tensor(0.0, device=self.device)
            self._cube_root_states[i, 3:7] = quat_mul(quat_from_euler_xyz(rand_r_box, rand_p_box, rand_yaw_box), self.init_quat[i])
            
            # 重置速度
            self._cube_root_states[i, 7:13] = 0. 
