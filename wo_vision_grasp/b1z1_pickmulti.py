...
class B1Z1PickMulti(B1Z1Base):
    ...
    def _init_tensors(self):
        ...
        if self.pred_success:
            self.predlift_reset_obj = torch.zeros(self.num_envs, device=self.device, dtype=torch.long)
    ...
    def _create_envs(self):
        ...
        # table
        self.table_dimz = 0.6
        self.table_dims = gymapi.Vec3(0.6, 0.6, self.table_dimz)
    ...
    def _compute_robot_obs(self, env_ids=None):
        if env_ids is None:
            ...
            table_dim = torch.tensor([0.6, 0.6, self.table_dimz]).repeat(self.num_envs, 1).to(self.device)
            ...
            table_dim = torch.tensor([0.6, 0.6, self.table_dimz]).repeat(len(env_ids), 1).to(self.device)
    ...
    def check_termination(self):
        ...
        if self.enable_camera:
            ...
            if too_close_table.any():
                print("终止原因：机器人过于靠近桌子")
